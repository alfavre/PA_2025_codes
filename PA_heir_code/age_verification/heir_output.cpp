...
CiphertextT age_verification(CryptoContextT cc, CiphertextT ct, CiphertextT ct1) {
  std::vector<int64_t> v0 = {1, 0};
  const auto& ct2 = cc->EvalSub(ct, ct1);
  ...
  const auto& ct3 = cc->EvalMult(ct2, pt);
  const auto& ct4 = cc->EvalRotate(ct3, 0);
  const auto& ct5 = ct4;
  const auto& ct6 = cc->ModReduce(ct5);
  return ct6;
}
